<?php

declare(strict_types=1);

namespace Tests\Unit;

use App\Core\App\KernelCli;
use Illuminate\Container\Container;
use PHPUnit\Framework\TestCase;
use App\Core\Repositories\Fields\FieldGroupRepositoryInterface;
use App\Core\Models\Fields\FieldGroup;
use Illuminate\Support\Collection;

class FieldGroupRepositoryTest extends TestCase
{
    private FieldGroupRepositoryInterface $repository;
    private KernelCli $kernel;
    private Container $container;
    private $fieldGroup = [
        "field_group_id" => 2,
        "type" => "product",
        "status" => 1
    ];


    protected function setUp(): void
    {
        parent::setUp();
        
        // Initialize the CLI kernel
        $this->kernel = new KernelCli();
        $this->container = $this->kernel->getContainer();
        $this->repository = $this->container->make(FieldGroupRepositoryInterface::class);
    }
    // public function testGetAll(): void
    // {
    //     $result = $this->repository->getAll();
        
    //     $this->assertIsArray($result);
    //     $this->assertInstanceOf(Collection::class, $result['items']);
    //     $this->assertIsInt($result['total']);
    //     $this->assertIsInt($result['total_pages']);
    //     $this->assertIsInt($result['current_page']);
    //     $this->assertIsInt($result['per_page']);
    // }
    public function testCreate(){
        $result = $this->repository->create($this->fieldGroup);
        $this->assertInstanceOf(FieldGroup::class, $result);
    }
    public function testFind(){
        $result = $this->repository->find($this->fieldGroup['field_group_id']);
        $this->assertInstanceOf(FieldGroup::class, $result);
    }
    public function testFindAll(){
        $result = $this->repository->findAll();
        $this->assertIsArray($result);
    }
    public function testUpdate(){
        $result = $this->repository->update($this->fieldGroup['field_group_id'], ['type' => 'product']);
        $this->assertInstanceOf(FieldGroup::class, $result);
        $this->assertEquals('product', $result->type);
    }
    // public function testDelete(){
    //     $result = $this->repository->delete($this->fieldGroup['field_group_id']);
    //     $this->assertTrue($result);
    // }

    
    

    protected function tearDown(): void
    {
        
        // Reset the kernel for the next test
        $this->kernel->reset();
        
        parent::tearDown();
    }
} 