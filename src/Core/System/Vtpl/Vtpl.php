...
namespace App\Core\System\Vtpl;
...
use function App\Core\System\utils\dotToArrayKey;
use function App\Core\System\utils\exceptionToArray;
use function App\Core\System\utils\filter;
use function App\Core\System\utils\logError;
use function App\Core\System\utils\prepareJson;
use function App\Core\System\utils\removeJsonComments;
use function App\Core\System\utils\stripExtraSpaces;

defined('VTPL_DEBUG') || define('VTPL_DEBUG', false);

defined('VTPL_HTML_MINIFY') || define('VTPL_HTML_MINIFY', false);
defined('VTPL_JS_MINIFY') || define('VTPL_JS_MINIFY', false);
defined('VTPL_PHP_MINIFY') || define('VTPL_PHP_MINIFY', false);
defined('VTPL_CLEAN_COMP_OPT') || define('VTPL_CLEAN_COMP_OPT', false);

defined('VTPL_DEBUG_SHOW_XPATH') || define('VTPL_DEBUG_SHOW_XPATH', true);
defined('VTPL_DONT_ALLOW_PHP') || define('VTPL_DONT_ALLOW_PHP', true); //don't set to false unless ABSOLUTELY NECESSARY!

if (!function_exists('vtplpostNameToArrayKey')) {
    include 'macros.php';
}
...
class Vtpl {
	...
	private $translationFunction = 'App\Core\System\utils\__';
	...
	function __construct($selector = null, $componentId = null, $componentContent = null) {
		...
		$this->document = \Dom\HTMLDocument::createEmpty();
		
		// Set document properties using proper methods to avoid deprecation warnings
		if (method_exists($this->document, 'setPreserveWhiteSpace')) {
			$this->document->setPreserveWhiteSpace(true);
		}
		if (method_exists($this->document, 'setRecover')) {
			$this->document->setRecover(true);
		}
		if (method_exists($this->document, 'setStrictErrorChecking')) {
			$this->document->setStrictErrorChecking(false);
		}
		if (method_exists($this->document, 'setSubstituteEntities')) {
			$this->document->setSubstituteEntities(false);
		}
		if (method_exists($this->document, 'setFormatOutput')) {
			$this->document->setFormatOutput(false);
		}
		if (method_exists($this->document, 'setResolveExternals')) {
			$this->document->setResolveExternals(false);
		}
		if (method_exists($this->document, 'setValidateOnParse')) {
			$this->document->setValidateOnParse(false);
		}
		if (method_exists($this->document, 'setXmlStandalone')) {
    ...
    function setTemplatePath($paths) {
	...
	function loadTemplateFileFromPath($templateFile, $extra = false) {
		...
		if (! $this->template || !$loaded) {
			$this->debug->log('LOAD', '<b>EMPTY</b> No template file found in any path for: ' . $templateFile);
	...
	function cssToXpath($selector) {
		...
		$cssSelector = [
			...
			// E:first-of-type: Matches the first element of type E
			'/([a-z]\w*):first-of-type/',	//	\1[1]
			
			// E:nth-of-type(n): Matches the nth element of type E
			'/([a-z]\w*):nth-of-type\((\d+)\)/',	//	\1[\2]
			
			// E:first-of-type in complex selectors
			'/:first-of-type/',	//	[1]
			
			// E:nth-of-type(n) in complex selectors
			'/:nth-of-type\((\d+)\)/',	//	[\1]
		...
		$xpathQuery = [
			...
			'\1[1]', //element:first
			'\1[\2]', //element:nth(2)
			'\1[1]', //element:first-of-type
			'\1[\2]', //element:nth-of-type(n)
			'[1]', //:first-of-type
			'[\1]', //:nth-of-type(n)
		...
		// Convert to namespace-aware XPath for modern Dom\HTMLDocument
		// Replace element names with local-name() function to work with namespaces
		$result = preg_replace('/\/([a-zA-Z][a-zA-Z0-9]*)/', '/*[local-name()="$1"]', $result);
		...
		if (isset($this->selectors) && isset($this->document) && isset($this->xpath)) {
			...
			foreach ($this->selectors as &$data) {
				...
				if ($selector == 'debug') {
					...
					// Skip problematic selectors that cause XPath errors
					if (
						strpos($selector, ':nth-of-type') !== false ||
						strpos($selector, ':first-of-type') !== false ||
						strpos($selector, ':last-of-type') !== false ||
						strpos($selector, '(') !== false ||
						strpos($selector, "'") !== false ||
						strpos($selector, '"') !== false ||
						strpos($selector, 'console.') === 0 ||
						strpos($selector, 'replace_php_code') !== false ||
						strpos($selector, '!') !== false ||
						preg_match('/[^a-zA-Z0-9_\-\.#\[\]=:> ]/', $selector) || // skip selectors with unusual characters
						preg_match('/\s{2,}/', $selector) // skip selectors with multiple spaces (likely not valid)
					) {
						$this->debug->log('WARNING', "Skipping invalid or problematic selector '$selector'");
						$elements = null;
					} else {
						$xpathSelector = $this->cssToXpath($selector);
						try {
							...
							$this->debug->log('WARNING', "XPath query failed for selector '$selector': " . $e->getMessage());
							$elements = null;
						}
					}
	...
	private function processAttributeConstants($value, $node) {
		...
		// Handle NodeList - use the first node
	...
	/**
	 * Remove all children from a node using modern Dom\ classes
	 * 
	 * @param Dom\Element $node - The node to remove children from
	 */
	private function removeChildren(&$node) {
		// Use the modern approach: remove all children efficiently
	...
	/**
	 * Get or set the innerHTML of elements using modern Dom\ classes
	 * 
	 * @param mixed $nodeList - DOMNodeList or array of nodes
	 * @param mixed $html - HTML content to set, or false to get current innerHTML
	 * @return string|void - Returns innerHTML string when getting, void when setting
	 */
	private function innerHTML($nodeList, $html = false) {
		...
		foreach ($nodeList as $node) {
			if ($html === false) {
				// GET innerHTML - Return the HTML content of the node
				return $this->getInnerHTML($node);
			} else {
				// SET innerHTML - Set the HTML content of the node
	...
	/**
	 * Get the innerHTML of a single node using modern Dom\ classes
	 * 
	 * @param Dom\Element $node - The node to get innerHTML from
	 * @return string - The innerHTML content
	 */
	private function getInnerHTML($node) {
		// Use the innerHTML property if available (for HTMLElement)
		if (property_exists($node, 'innerHTML')) {
			return $node->innerHTML;
		}
		
		// Fallback for XML elements or other cases
		if ($this->documentType == 'html') {
			$tempDoc = \Dom\HTMLDocument::createEmpty();
		} else {
			$tempDoc = \Dom\XMLDocument::createEmpty();
		}
		
		// Copy all child nodes to the temporary document
		foreach ($node->childNodes as $child) {
			$importedNode = $tempDoc->importNode($child, true);
			$tempDoc->appendChild($importedNode);
		}

		// Return the HTML content based on document type
		if ($this->documentType == 'html') {
			return $tempDoc->saveHtml();
		} else {
			return $tempDoc->saveXML();
		}
	}

	/**
	 * Set the innerHTML of a single node using modern Dom\ classes
	 * 
	 * @param Dom\Element $node - The node to set innerHTML for
	 * @param string $html - The HTML content to set
	 */
	private function setInnerHTML($node, $html) {
		if ($html == '') {
			return;
		}

		// Handle external elements (from external HTML files)
		if ($this->_external_elements) {
			$this->setInnerHTMLFromExternal($node, $html);
			return;
		}

		// Handle special element types that don't support innerHTML
		switch ($node->nodeName) {
			...
			default:
				$this->setInnerHTMLForStandardElement($node, $html);
				break;
		}
	}

	/**
	 * Set innerHTML for external elements (from external HTML files)
	 * 
	 * @param Dom\Element $node - The target node
	 * @param string $html - The external HTML reference
	 */
	private function setInnerHTMLFromExternal($node, $html) {
		...
		if ($result) {
			...
			// Import and append external nodes
	...
	/**
	 * Set innerHTML for standard HTML elements
	 * 
	 * @param Dom\Element $node - The target node
	 * @param string $html - The HTML content to set
	 */
	private function setInnerHTMLForStandardElement($node, $html) {
		// Remove existing children
		$this->removeChildren($node);
		
		// Process attribute constants in the HTML
		$processedHtml = $this->processAttributeConstants($html, $node);
		
		// Create document fragment using modern Dom\ classes
		$fragment = $this->document->createDocumentFragment();
		
		try {
			// Append the processed HTML to the fragment
			$fragment->appendXML($processedHtml);
			
			// Append the fragment to the node
			$node->appendChild($fragment);
		} catch (Exception $e) {
			// Handle XML parsing errors gracefully
			$this->debug->log('ERROR', 'Failed to parse HTML: ' . $e->getMessage());
			
			// Fallback: create a text node with the raw HTML
			$textNode = $this->document->createTextNode($processedHtml);
			$node->appendChild($textNode);
		}
	}

	private function outerHTML(&$nodeList, $html = false) {
		if (!$this->validateNodeList($nodeList)) {
			return;
		}
		foreach ($nodeList as $node) {
			if ($html === false) {
				// Use modern Dom\HTMLDocument instead of old DOMDocument
				if ($this->documentType == 'html') {
	...
	/**
	 * Helper function to safely iterate over nodeList
	 * 
	 * @param mixed $nodeList - NodeList to iterate over
	 * @return bool - True if nodeList is valid and iterable
	 */
	private function validateNodeList($nodeList) {
		return $nodeList && (is_array($nodeList) || is_object($nodeList));
	}
	...
	private function setAttribute(&$nodeList, $attribute, $val) {
		if (is_a($nodeList,'DOMNodeList') || is_a($nodeList,'\Dom\NodeList')) {
	...
	/**
	 * Load HTML content from external files using modern Dom\ classes
	 * 
	 * @param mixed $val - External HTML reference
	 * @param Dom\Element $node - The context node
	 * @return Dom\NodeList|false - NodeList of elements or false on failure
	 */
	private function loadFromExternalHtml($val, $node) {
		...
		$html = $this->processExternalHtmlContent($html);

		// Create document using modern Dom\ classes
		$document = $this->createExternalDocument($html);
		if (!$document) {
			return false;
		}

		// Query elements using modern Dom\XPath
	...
	/**
	 * Resolve the full path for external HTML files
	 * 
	 * @param string $filename - The filename to resolve
	 * @return string - The resolved full path
	 */
	private function resolveExternalFilePath($filename) {
		if (substr_compare($filename, '/plugins/', 0, 9) === 0) {
			return DIR_ROOT . $filename;
		} elseif (substr_compare($filename, '/public/', 0, 8) === 0) {
			return DIR_ROOT . $filename;
		} elseif ($filename[0] !== '/') {
			return $this->htmlPath . $filename;
		}
	 ...
	 * 
	 * @param string $filename - The full path to the HTML file
	 * @return string|false - HTML content or false on failure
	 */
	private function loadExternalHtmlContent($filename) {
		$html = @file_get_contents($filename);
		
		if ($html === false) {
			logError("can't load html $filename");
			$this->debug->log('LOAD', '<b>EXTERNAL ERROR</b> ' . $filename);
			return false;
		}
	...
	/**
	 * Process external HTML content (remove PHP, minify, etc.)
	 * 
	 * @param string $html - Raw HTML content
	 * @return string - Processed HTML content
	 */
	private function processExternalHtmlContent($html) {
		// Remove PHP code if not allowed
	...
	/**
	 * Create a document from HTML content using modern Dom\ classes
	 * 
	 * @param string $html - HTML content
	 * @return Dom\HTMLDocument|Dom\XMLDocument|null - Document object or null on failure
	 */
	private function createExternalDocument($html) {
		try {
			if ($this->documentType == 'html') {
						// Use modern Dom\HTMLDocument for HTML content
		return \Dom\HTMLDocument::createFromString($html);
			} else {
				// Use modern Dom\XMLDocument for XML content
				return Dom\XMLDocument::createFromString($html);
			}
		} catch (Exception $e) {
			$this->debug->log('ERROR', 'Failed to create document from external HTML: ' . $e->getMessage());
			return null;
		}
	}

	/**
	 * Load HTML content into the document using modern Dom\ classes
	 * 
	 * @param string $html - HTML content to load
	 * @return array - Array of libxml errors
	 */
	function loadHtml($html) {
		...
		// Create document using modern Dom\ classes
		$errors = $this->createDocumentFromHtml($html);
		
		// Initialize XPath using modern Dom\XPath
		$this->xpath = new \Dom\XPath($this->document);
		
		// Register namespaces for HTML elements
		$this->xpath->registerNamespace('html', 'http://www.w3.org/1999/xhtml');
		
		// Handle component content replacement
		$this->handleComponentContent();
		
		// Add base tag if missing
		$this->addBaseTagIfMissing();
		
		return $errors;
	}

	/**
	 * Process HTML content before loading into document
	 * 
	 * @param string $html - Raw HTML content
	 * @return string - Processed HTML content
	 */
	private function processHtmlContent($html) {
		// Remove PHP code if not allowed
		if (VTPL_DONT_ALLOW_PHP) {
			$html = $this->removePhp($html);
		}
		...
		$html = $this->processScriptTags($html);
	...
	/**
	 * Process script tags and store them for later restoration
	 * 
	 * @param string $html - HTML content with script tags
	 * @return string - HTML content with script placeholders
	 */
	private function processScriptTags($html) {
	...
	/**
	 * Create document from HTML content using modern Dom\ classes
	 * 
	 * @param string $html - Processed HTML content
	 * @return array - Array of libxml errors
	 */
	private function createDocumentFromHtml($html) {
		try {
			if ($this->documentType == 'html') {
				// Use modern Dom\HTMLDocument for HTML content
				$this->document = \Dom\HTMLDocument::createFromString($html);
			} else {
				// Handle JSON conversion if needed
				if ($this->documentType == 'json') {
					$html = $this->convertJsonToXml($html);
				}
				
				// Use modern Dom\XMLDocument for XML content
				$this->document = Dom\XMLDocument::createFromString($html);
			}
			...
			$this->debug->log('ERROR', 'Failed to create document from HTML: ' . $e->getMessage());
			return [new LibXMLError()];
		}
	}
	 ...
	 */
	private function convertJsonToXml($jsonContent) {
	...
	/**
	 * Handle component content replacement using modern Dom\ classes
	 */
	private function handleComponentContent() {
		...
		if ($elements && $elements->length > 0) {
			...
			if ($node && $node->parentNode) {
				// Create temporary document using modern Dom\ classes
				$tmpDoc = $this->createTemporaryDocument($this->componentContent);
				
				if ($tmpDoc) {
					$body = $tmpDoc->getElementsByTagName('body');
					
					if ($body->length > 0 && $body->item(0)->firstChild) {
	...
	/**
	 * Create a temporary document from content using modern Dom\ classes
	 * 
	 * @param string $content - HTML/XML content
	 * @return Dom\HTMLDocument|Dom\XMLDocument|null - Document object or null on failure
	 */
	private function createTemporaryDocument($content) {
		try {
			if ($this->documentType == 'html') {
				return \Dom\HTMLDocument::createFromString($content);
			} else {
				return Dom\XMLDocument::createFromString($content);
			}
		} catch (Exception $e) {
			$this->debug->log('ERROR', 'Failed to create temporary document: ' . $e->getMessage());
			return null;
		}
	}

	/**
	 * Add base tag if missing using modern Dom\ classes
	 */
	private function addBaseTagIfMissing() {
	...
	function loadHtmlTemplate($filename) {
		...
		if (! ($html = @file_get_contents($filename))) {
			logError("can't load template $filename");
			$this->debug->log('LOAD', '<b>ERROR</b> ' . $filename);

			return ['Failed to load template file: ' . $filename];
		}
	...
	private function setMultiLanguageText($currentNode) {
		...
		while ($i < $length) {
			...
			// Skip DocumentType nodes (DOCTYPE declarations)
			...
			if ($node && $node->nodeType == XML_ELEMENT_NODE && $node->hasAttributes()) {
				foreach ($node->attributes as $name => $attrNode) {
			...
			if ($node && $node->nodeType == XML_TEXT_NODE &&
				...
				if ($trimmed != '') {
					if (strlen($trimmed) < 1024) {
						try {
							$c = $this->document->createTextNode('echo ' . $this->translationFunction . '(\'' . addcslashes($trimmed, "'") . '\');');
							...
							// Update length after DOM manipulation
							$length = $currentNode->childNodes->length;
						} catch (Exception $e) {
		...
		*/
		...
		$html = preg_replace_callback('/{\s*(\$[\w\-\>\.]+)\|?([\w\.]+)?\s*}/',
					  function ($matches) use ($self) {
					  	...
					  	$variable = dotToArrayKey($variable);
					  	$template =
		...
		$html = $this->fixHtmlEntities($html);
		return $html;
	...
	/**
	 * Save compiled template to file using modern Dom\ classes
	 * 
	 * @param string $compiledFile - Path to save the compiled template
	 * @return bool - True on success, false on failure
	 */
	function saveCompiledTemplate($compiledFile) {
		try {
			// Process template and generate HTML
			$html = $this->generateCompiledHtml();
			
			if (empty($html)) {
				logError("compiled template is empty for $compiledFile");
				return false;
			}
			...
			return $this->saveTemplateToFile($compiledFile, $html);
			
		} catch (Exception $e) {
			$this->debug->log('ERROR', 'Failed to save compiled template: ' . $e->getMessage());
			logError("Failed to save compiled template to $compiledFile: " . $e->getMessage());
			return false;
		}
	}

	/**
	 * Generate compiled HTML content using modern Dom\ classes
	 * 
	 * @return string - Generated HTML content
	 */
	private function generateCompiledHtml() {
		try {
			// Process template file and template
			$this->processTemplateFile();
			$this->processTemplate();
			$this->setMultiLanguageText($this->document);

			if ($this->selector) {
				return $this->generateSelectorBasedHtml();
			} else {
				return $this->generateFullDocumentHtml();
			}
		} catch (Exception $e) {
			throw $e;
		}
	...
	/**
	 * Generate HTML for specific selector using modern Dom\ classes
	 * 
	 * @return string - Generated HTML content
	 */
	private function generateSelectorBasedHtml() {
		$elements = $this->xpath->query($this->cssToXpath($this->selector));
		...
		// Create temporary document using modern Dom\ classes
		$tmpDoc = $this->createTemporaryDocumentForComponent($componentNode);
		...
		return $this->generateHtmlFromDocument($tmpDoc);
	}

	/**
	 * Create temporary document for component with script tags using modern Dom\ classes
	 * 
	 * @param Dom\Element $componentNode - The component node
	 * @return Dom\HTMLDocument|Dom\XMLDocument|null - Temporary document or null on failure
	 */
	private function createTemporaryDocumentForComponent($componentNode) {
		try {
			// Create temporary document using modern Dom\ classes
			$tmpDoc = $this->createEmptyTemporaryDocument();
			
			// Add script tags before component
			$this->addScriptTagsBeforeComponent($tmpDoc, $componentNode);
			
			// Add component node
			$importedComponent = $tmpDoc->importNode($componentNode, true);
			$tmpDoc->appendChild($importedComponent);
			
			// Add script tags after component
			$this->addScriptTagsAfterComponent($tmpDoc, $componentNode);
	...
	/**
	 * Create empty temporary document using modern Dom\ classes
	 * 
	 * @return Dom\HTMLDocument|Dom\XMLDocument - Empty document
	 */
	private function createEmptyTemporaryDocument() {
		if ($this->documentType == 'html') {
			return \Dom\HTMLDocument::createEmpty();
		} else {
	...
	/**
	 * Add script tags before component node
	 * 
	 * @param Dom\Document $tmpDoc - Temporary document
	 * @param Dom\Element $componentNode - Component node
	 */
	private function addScriptTagsBeforeComponent($tmpDoc, $componentNode) {
		$currentNode = $componentNode;
		
		while (($currentNode = $currentNode->previousSibling) && $currentNode->nodeName === '_script') {
			$importedNode = $tmpDoc->importNode($currentNode, true);
			$tmpDoc->appendChild($importedNode);
		}
	}

	/**
	 * Add script tags after component node
	 * 
	 * @param Dom\Document $tmpDoc - Temporary document
	 * @param Dom\Element $componentNode - Component node
	 */
	private function addScriptTagsAfterComponent($tmpDoc, $componentNode) {
		$currentNode = $componentNode;
	...
	/**
	 * Generate HTML from full document using modern Dom\ classes
	 * 
	 * @return string - Generated HTML content
	 */
	private function generateFullDocumentHtml() {
		return $this->generateHtmlFromDocument($this->document);
	}

	/**
	 * Generate HTML from document using modern Dom\ classes
	 * 
	 * @param Dom\Document $document - Document to generate HTML from
	 * @return string - Generated HTML content
	 */
	private function generateHtmlFromDocument($document) {
		try {
			if ($this->documentType == 'html') {
				// Use saveHtml() (lowercase 'h') for new Dom\HTMLDocument
				$html = $document->saveHtml();
			} else {
				$html = $document->saveXML();
			}
			
			// Fix HTML entities that were incorrectly converted from PHP code
			// This replaces the old substituteEntities = false behavior
			
			return trim($html);
			
		} catch (Exception $e) {
			$this->debug->log('ERROR', 'Failed to generate HTML: ' . $e->getMessage());
			return '';
		}
	}

	/**
	 * Save template to file with syntax validation
	 * 
	 * @param string $compiledFile - Path to save file
	 * @param string $html - HTML content to save
	 * @return bool - True on success, false on failure
	 */
	private function saveTemplateToFile($compiledFile, $html) {
		if ($this->checkSyntax && function_exists('token_get_all')) {
			return $this->saveWithSyntaxValidation($compiledFile, $html);
		} else {
			return $this->saveWithoutValidation($compiledFile, $html);
		}
	}

	/**
	 * Save template with PHP syntax validation
	 * 
	 * @param string $compiledFile - Path to save file
	 * @param string $html - HTML content to save
	 * @return bool - True on success, false on failure
	 */
	private function saveWithSyntaxValidation($compiledFile, $html) {
		try {
			// Write file first
			file_put_contents($compiledFile, $html);
			
			// Validate PHP syntax - ensure $html is not null
			...
			// Handle syntax error
			$data = exceptionToArray($e, $compiledFile);
			
			try {
				Response::fromException(app()->make(InternalServerErrorHttpException::class));
			...
			$this->debug->log('ERROR', 'Failed to save with validation: ' . $e->getMessage());
			return false;
		}
	}

	/**
	 * Save template without validation
	 * 
	 * @param string $compiledFile - Path to save file
	 * @param string $html - HTML content to save
	 * @return bool - True on success, false on failure
	 */
	private function saveWithoutValidation($compiledFile, $html) {
		try {
			file_put_contents($compiledFile, $html);
			return true;
		} catch (Exception $e) {
			$this->debug->log('ERROR', 'Failed to save file: ' . $e->getMessage());
			return false;
		}
	}
	 ...
	 * @return string - Fixed HTML content
	 */
	private function fixHtmlEntities($html) {
		// Convert common HTML entities back to their original characters
		// This prevents PHP code from being converted to HTML entities
		$replacements = [
			'&gt;' => '>',  // Greater than
			'&lt;' => '<',  // Less than
			'&amp;' => '&', // Ampersand
		...
		// Only replace entities that are part of PHP code
		$html = preg_replace_callback('/<\?php.*?\?>/s', function($matches) use ($replacements) {
			$phpCode = $matches[0];
			foreach ($replacements as $entity => $char) {
				$phpCode = str_replace($entity, $char, $phpCode);
			}
			return $phpCode;
		}, $html);
