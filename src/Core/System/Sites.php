...
use function App\Core\System\Utils\getConfig;
	...
	*/
	...
	public static function urlSplit($url = null) {
		...
		if (preg_match(self::HOST_REGEX, $url, $matches)) {
			...
			if (is_numeric($host_matches['domain'] ?? null) || $host == 'localhost' || (!str_contains($host, '.'))) {
				$matches['domain']    = $host;
	...
	public static function getSiteData($site_url = false) {
		...
		$result = config("sites.$host", null) ??
				  config("sites.$subdomain_wildcard", null) ??
				  config("sites.$domain_wildcard", null) ??
				  config("sites.$full_wildcard", null) ??
				  config("sites.$tld_wildcard", null) ??
				  config('sites.* * *', null);
	...
	public static function saveSite($site) {
		...
		unset($site['key']);
		$return = setConfig("sites.$key", $site);
	}

	public static function deleteSite($site) {
		$key    = self::siteKey(trim($site['key'] ?? $site['host']));
		$return = unsetConfig("sites.$key", $site);
	}
}
