...
use function App\Core\System\Utils\getDefaultTemplateList;

class Themes extends Extensions {
	static protected string $extension = 'theme';

	static protected string $baseDir = DIR_THEMES;

	static protected ?string $url = null;

	static protected ?string $feedUrl = null;

	static protected array $themes = [];

	static protected array $categories = [];

    static function getThemePath(): string
    {
        return DIR_THEME . Sites::getTheme() ?? 'default';
    }

	static function getInfo($content, $name = false): array
    {
	...
	*/
	...
	static function install($extensionZipFile, $slug = false, $validate = false): bool|string
    {
