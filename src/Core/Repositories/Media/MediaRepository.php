...
class MediaRepository extends BaseRepository implements MediaRepositoryInterface
{
    ...
    private const UPLOAD_MAX_FILE_MB = 45;
    ...
    public function uploadFile(array $data, ?int $folder_id = null, ?string $folder_path = null): ?array
    {
        ...
        foreach($files as $file){
            ...
            if(empty($e)){
                $relPath = DS . $uploadDirName . DS . $filename;
                $fileJson = $this->prepareFileJson(
                    $relPath,
                    $filename,
                    $file['type'],
                    (int) $file['size'],
                ...
                $filesData[] = $this->buildStoredMediaRow($fileJson, $file['name'], null, $folder_id);
                $filesResponseData[] = [
                    'name' => $file['name'],
    ...
    /**
     * Upload image or non-image media: validate, save to disk, then route raster images through `uploadImageFile()`
     * (WebP + optional canvas) or everything else through `uploadOtherFiles()` (no resize; zip/Open XML may shrink).
     *
     * Flow per image: (1) validate and save → (2) if no target W/H, same-size WebP when possible → (3) if
     * `$size['width']` and `$size['height']` are set, `processImageWithSize()` (skips step 2) → (4) persist rows.
     *
     * Non-image MIME types in `$otherFilesTypes` skip WebP and resizing; optional zip repack applies where supported.
     */
    public function upload(array $data, ?array $size = [], ?string $folder_path = null, ?int $folder_id = null, $is_banner = false, int $file_max_size = 65): ?array
    {
        //get the file from the request
        if (!isset($data['files'])) {
            return null;
        ...
        $otherFilesTypes = [
            ...
            'application/pdf',
            'application/zip',
            // Autodesk/Sketchup/Revit/other drawing types
            'application/max',
            'application/dwg',
            'application/skp',
            'application/rfa',
            ...
            // ALL DOCX FILE MIME TYPES (WORD DOCS)
            'application/msword', // .doc
            'application/doc',    // rarely seen, but some old systems
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document', // .docx
            'application/vnd.openxmlformats-officedocument.wordprocessingml.template', // .dotx
            'application/vnd.ms-word.document.macroEnabled.12', // .docm
            'application/vnd.ms-word.template.macroEnabled.12', // .dotm
            // Non-standard client reports (also listed on `$otherFiles`); must be here or validation rejects before `uploadOtherFiles`
            'application/docx',
            'application/zipdocx',
            ...
            // ICF File Configurations
            'text/plain',              // Frequently assigned to .icf text/router configs
            'application/lcf',
            'application/x-lcf',
            // Custom fallback
        ];

        $acceptedMimeTypes = array_merge($allowedTypes, $otherFilesTypes);
        ...
        foreach ($f as $file) {
            ...
            if (isset($file['type']) && !in_array($file['type'], $acceptedMimeTypes, true)) {
                $e['file_type'] = 'File type not allowed';
            }
    
            $maxAllowedBytes = 1024 * 1024 * $file_max_size;
            if (isset($file['size']) && (int) $file['size'] > $maxAllowedBytes) {
                $uploadedFileSizeMb = number_format(((int) $file['size'] / (1024 * 1024)), 3, '.', '');
                $e['file_size'] = 'File size is too large. Your file size is ' . $uploadedFileSizeMb . 'MB. Maximum size is ' . $file_max_size . 'MB.';
            }
            ...
            if (empty($e)) {
                try {
                    if (in_array($file['type'], $allowedTypes, true)) {
                        ...
                        $file = $built['file'];
                    } elseif (in_array($file['type'], $otherFilesTypes, true)) {
                        $built = $this->uploadOtherFiles($file, $filepath, $uploadDirName, $folder_id);
                        $file = $built['file'];
                    } else {
                        $e['file_type'] = 'File type not allowed';
                    }
                } catch (\Throwable $th) {
                    $e['file_process'] = $th->getMessage();
                }
            }

            if (empty($e) && $built !== null) {
                $filesData[] = $built['filesData'];
                $filesResponseData[] = $built['filesResponseData'];
                $path[] = $built['path'];
            } else {
                $error[$file['name']] = $e;
                $path[] = '';
        ...
        try{
            if(count($filesData)){
               ...
               $pathsForLookup = array_values(array_unique(array_filter($path, static fn ($p) => $p !== '')));
               $mediaIds = $this->model->whereIn('path', $pathsForLookup)->select(['media_id', 'path'])->limit(0)->findAll();
    ...
    /**
     * Payload stored in media `file` JSON (path, name, type, size, mime, URL, optional dimensions / targets).
     *
     * @param mixed|null $dimensions
     *
     * @return array<string, mixed>
     */
    private function prepareFileJson(
        string $relPath,
        string $basename,
        string $mimeType,
        int $storedSize,
        ...
        bool $includeDimensionsAndTargets = true
    ): array {
        $data = [
            'path' => $relPath,
            'name' => $basename,
        ...
        if ($includeDimensionsAndTargets) {
            $data['dimensions'] = $dimensions;
            $data['target_width'] = $targetWidth;
            $data['target_height'] = $targetHeight;
        }
    ...
    /**
     * One `files[]` API item aligned with {@see prepareFileJson()} plus the raw upload `file` array.
     *
     * @param array<string, mixed> $fileJson
     *
     * @return array<string, mixed>
     */
    private function prepareUploadFilesResponseFromFileJson(array $file, array $fileJson): array
    {
        return [
            'path' => $fileJson['path'],
            'name' => $fileJson['name'],
            ...
            'size' => $fileJson['size'],
            'type' => $fileJson['type'],
            'objectURL' => $fileJson['objectURL'],
            'created_at' => date('Y-m-d'),
            'file' => $file,
            'dimensions' => $fileJson['dimensions'] ?? null,
            'target_size' => [
                'width' => $fileJson['target_width'] ?? null,
                'height' => $fileJson['target_height'] ?? null,
            ],
            'status' => ['name' => 'Uploaded', 'severity' => 'success'],
            'media_id' => null,
        ];
    ...
    /**
     * Normalize MIME type to a short media `type` value that fits DB constraints.
     */
    private function normalizeTypeForStorage(string $mimeType, ?string $filename = null): string
    {
        ...
        $mapped = match ($normalized) {
            ...
            'video/mp4' => 'mp4',
            'application/pdf' => 'pdf',
            'application/zip', 'application/x-zip-compressed', 'multipart/x-zip', 'application/x-compressed' => 'zip',
            'application/msword', 'application/doc' => 'doc',
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/docx', 'application/zipdocx' => 'docx',
            'application/vnd.openxmlformats-officedocument.wordprocessingml.template' => 'dotx',
            'application/vnd.ms-word.document.macroenabled.12' => 'docm',
            'application/vnd.ms-word.template.macroenabled.12' => 'dotm',
            ...
            'text/csv', 'application/csv', 'application/x-csv', 'text/comma-separated-values', 'text/x-comma-separated-values' => 'csv',
            'application/max' => 'max',
            'application/dwg', 'image/vnd.dwg', 'application/x-dwg' => 'dwg',
            'image/vnd.dxf', 'application/x-dxf' => 'dxf',
            'application/skp' => 'skp',
            'application/rfa', 'application/x-rfa' => 'rfa',
            default => null,
        };
        ...
        if ($extension !== '') {
            return substr($extension, 0, 30);
        }

        return substr(str_replace(['/', '.'], '_', $normalized), 0, 30);
    }

    /**
     * Build one media table row: JSON `file` + `meta` payloads and scalar columns used by upsert.
     *
     * @param array<string, mixed> $fileJson from {@see prepareFileJson()}
     * @param int|null $originalUploadSize When null, `original_size` is omitted from `meta` (legacy minimal rows).
     *
     * @return array{file: string, path: string, type: string, folder_id: int|null, meta: string}
     */
    private function buildStoredMediaRow(
        array $fileJson,
        string $originalNameForMeta,
        ?int $originalUploadSize,
        ?int $folderId
    ): array {
        $mimeType = $fileJson['type'];
        $storedType = $this->normalizeTypeForStorage($mimeType, (string) ($fileJson['name'] ?? ''));
        $storedSize = (int) $fileJson['size'];
        $metaPayload = [
            'original_name' => $originalNameForMeta,
            'size' => $storedSize,
            'mime_type' => $mimeType,
        ];
        if ($originalUploadSize !== null) {
            $metaPayload['original_size'] = $originalUploadSize;
        }
        if (array_key_exists('dimensions', $fileJson)) {
            $metaPayload['dimensions'] = $fileJson['dimensions'];
            $metaPayload['target_width'] = $fileJson['target_width'];
            $metaPayload['target_height'] = $fileJson['target_height'];
        }

        return [
            'file' => json_encode($fileJson),
            'path' => $fileJson['path'],
            'type' => $storedType,
            'folder_id' => $folderId,
            'meta' => json_encode($metaPayload),
            'created_at' => date('Y-m-d')
        ];
    }

    /**
     * Raster image branch after `move_uploaded_file()`: same-size WebP when applicable, optional canvas WebP, media row.
     *
     * @return array{file: array, filesData: array, filesResponseData: array, path: string}
     */
    private function uploadImageFile(
        ...
        $originalClientName = $file['name'] ?? '';
        $originalUploadSize = (int) ($file['size'] ?? 0);
        ...
        if (in_array($file['type'], $allowedTypes, true)) {
            if ($targetWidth && $targetHeight) {
                ...
                if (!$processedImageData) {
                    throw new \RuntimeException('Failed to resize/convert image.');
                }
        ...
        if (!is_file($processedImagePath)) {
            throw new \RuntimeException('Final processed image file was not saved.');
        }
        $actualSavedSize = filesize($processedImagePath);
        if ($actualSavedSize !== false) {
            $processedImageSize = (int) $actualSavedSize;
            $file['size'] = $processedImageSize;
        }
        ...
        $filesData = $this->buildStoredMediaRow($fileJson, $originalClientName, $originalUploadSize, $folder_id);

        $filesResponseData = $this->prepareUploadFilesResponseFromFileJson($file, $fileJson);
    ...
    /**
     * Non-image media after `move_uploaded_file()`: store exactly as uploaded (no optimization, no resize).
     *
     * @return array{file: array, filesData: array, filesResponseData: array, path: string}
     */
    private function uploadOtherFiles(array $file, string $filepath, string $uploadDirName, ?int $folder_id): array
    {
        ...
        $processedSize = is_file($filepath) ? (int) filesize($filepath) : $originalUploadSize;
        $file['size'] = $processedSize;

        $imageDimensions = null;
        $targetWidth = null;
        $targetHeight = null;

        $basename = basename($processedPath);
        $relPath = DS . $uploadDirName . DS . $basename;

        $originalNameForMeta = $file['name'] ?? '';
        $fileJson = $this->prepareFileJson(
            $relPath,
            $basename,
            $file['type'],
        ...
        $filesData = $this->buildStoredMediaRow($fileJson, $originalNameForMeta, $originalUploadSize, $folder_id);
    ...
    public function uploadFiles(array $data, array $allowedTypes, ?string $folder_path = null, ?int $folder_id = null): ?array
    {
        ...
        foreach($data['files'] as $file){
            ...
            //prepare media data for media table to be inserted
            if(empty($e)){
                $relPath = DS . $uploadDirName . DS . basename($processedImagePath);
                $basename = basename($processedImagePath);
                $originalSize = (int) ($file['size'] ?? 0);
                $fileJson = $this->prepareFileJson(
                    $relPath,
                    $basename,
                    $file['type'],
                ...
                $filesData[] = $this->buildStoredMediaRow($fileJson, $file['name'], $originalSize, $folder_id);
                $filesResponseData[] = [
    ...
    /**
     * @param list<array{name: string, media_id?: int|null}> $files
     */
    public function deleteMediaByFileNames(string $path, array $files): bool
    {
        ...
        $folderPath = trim(str_replace('\\', '/', $path), '/');
        $publicRoot = ROOT_DIR . DS . 'public';
        $hadSuccess = false;
        ...
        foreach ($files as $file) {
            ...
            $fullFilePath = $publicRoot . DS . $folderPath . DS . $file['name'];
            if (is_file($fullFilePath) && @unlink($fullFilePath)) {
                $hadSuccess = true;
            }
            $mediaId = $file['media_id'] ?? null;
            if ($mediaId && $mediaId > 0 && $this->model->delete((int) $mediaId)) {
                $hadSuccess = true;
            }
        }
    ...
    /**
     * Values that may appear in `media.path` depending on OS / upload helpers (DS vs '/').
     *
     * @return list<string>
     */
    private function buildStoredPathCandidates(string $relativeUnix): array
    {
        $leading = '/' . $relativeUnix;
        $candidates = [
            $leading,
            $relativeUnix,
            str_replace('/', DS, $leading),
            str_replace('/', DS, $relativeUnix),
        ];
    ...
    public function getSubFoldersData(string $folderPath): array
    {
        ...
        foreach ($items as $item) {
            ...
            if (is_dir($fullPath)) {
                ...
                $fileCreatedAt = date('Y-m-d H:i:s', filemtime($fullPath));
                $files[] = [
                    'id' => null,
                    'data_type' => $dataType,
                    'file' => [
