...
class Categoriesslidernav extends ComponentBase {
    ...
    function results($params = []) {
        $component = $this->componentRepository->getComponentByName('categoriesslidernav');
        $component->items = array_map(function($item) {
            return $item['fields'];
        }, $component->items);
        list($component) = Event::trigger(__CLASS__,__FUNCTION__, $component);
        $component->options = $this->options;
        $data = $this->categoryRepository->getCategoriesForSliderNavComponent();
        $config = config('APP_ADMIN_URL');
        $data['component_link'] = $config."/components/{$component->component_id}/items";
