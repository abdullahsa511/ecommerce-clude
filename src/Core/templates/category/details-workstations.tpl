...
#category-workstation-hero | before = <?php 
	...
	$isSubCategoriesExist = isset($category_hero['categories']) && count($category_hero['categories']) > 0 ? true : false; 
	$categories = isset($category_hero['categories']) ? $category_hero['categories'] : [];
...
[data-v-categoryhero-component-link] | if_exists = $is_admin
[data-v-category-hero-edit-link] | if_exists = $is_admin
...
div[data-v-component-link] | if_exists = $is_admin


#workstations-categories | before = <?php 
    if(isset($current_component)) $previous_component = $current_component;
	$categories = $current_component = $this->_component['categoryworkstationdetails']?? [];
...
div[data-v-component-categoryworkstationdetails] > section | deleteAllButFirst


div[data-v-component-categoryworkstationdetails] | prepend = <?php if(isset($categories['sections'])){ foreach ($categories['sections'] as $key => $section) { ?>
div[data-v-category] | id = <?php echo "th-product-slider-".$key; ?>
h2[data-v-category-title] | innerText = <?php echo ucwords(strtolower(str_replace(['_', '-'], ' ', $section['title']??""))); ?>
div[data-v-category-subtitle] | innerText = <?php echo $section['subtitle']??""; ?>
a[data-v-category-link] | href = <?php echo $section['link']??""; ?>
div[data-v-category-products] > .swiper-slide | deleteAllButFirst
div[data-v-category-products] | prepend = <?php if(isset($section['items'])){ foreach ($section['items'] as $item) { ?>
	...
	div[data-v-category-product-tags] | prepend = <?php if(isset($item['tags'])){ foreach ($item['tags'] as $tag) { ?>
		div[data-v-category-product-tag] | innerText = <?php echo $tag??""; ?>
...
div[data-v-category-products] | append = <?php }} ?>
div[data-v-component-categoryworkstationdetails] | append = <?php }} ?>
