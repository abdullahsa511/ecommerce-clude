...
#category-hero | before = <?php 
	...
	// echo '<pre>';
	// print_r(count($category_hero['categories']));
	// echo '</pre>';
...
#category-details| before = <?php 
    if(isset($current_component)) $previous_component = $current_component;
	$categories = $current_component = $this->_component['categorydetails']?? [];
...
#category-details > .th-section | deleteAllButFirst
#category-details | prepend = <?php if(isset($categories['sections'])){ foreach ($categories['sections'] as $key => $section) { ?>
