...
class QuoteDataValidation
{
    ...
    public function __construct(array $data,  array $existingData = ["quoteMap" => [], 'quoteIds' => []])
    {
        $this->rawData = $data;
        ...
        if(isset($data['quote_id'])) {
            $this->quote->quote_id = $this->validateInteger($data['quote_id'], 'value', 0);
            if(isset($existingData['quoteIds'][$data['quote_id']])) {
                $this->isExistingData = true;
            }
        }
        // ATTRIBUTE TABLE
        if(isset($data['company_name']) && !empty($data['company_name']) && $data['company_name']){
            $this->quote->company_name = $this->validateString($data['company_name'], 'company_name', 191);
            $this->quote->reference_number = $this->validateInteger($data['reference_number'], 'reference_number') ?? null;
            
            
            $name = $this->quote->company_name;
            if(isset($existingData['quoteMap'][$name])){
                $this->quote->quote_id = $existingData['quoteMap'][$name];
                $this->isExistingData = true;
            }
            else{
                $this->quote->quote_id = $this->validateInteger($data['quote_id'] ?? Null, 'quote_id', 0);
                if(!count($this->errors)){
                    $this->quote->company_name = $name;
                    if(!$this->isExistingData){
                        $this->quote->company_name = $this->quote->company_name;
                    }
                }
            };
        }else{
            $this->addError('company Name', 'Quote company name is required');
        }
    }
    ...
    {
        if (!empty($this->quote->uuid)) {
            return 'quote_' . $this->quote->uuid;
        }
    ...
    public function getQuoteUniqueIdentifier(): string
    {
