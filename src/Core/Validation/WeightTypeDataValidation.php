...
class WeightTypeDataValidation
{
    ...
    public function __construct(array $data,  array $existingData = ["weightTypeMap" => [], 'weightTypeIds' => []])
    {
        $this->rawData = $data;

        $this->weightType = new stdClass();


        if(isset($data['weight_type_id'])) {
            $this->weightType->weight_type_id = $this->validateInteger($data['weight_type_id'], 'value', 0);
            if(isset($existingData['weightTypeIds'][$data['weight_type_id']])) {
                $this->isExistingData = true;
            }
        }
        // ATTRIBUTE TABLE
        if(isset($data['name']) && !empty($data['name']) && $data['name']){
            $this->weightType->name = $this->validateString($data['name'], 'name', 191);
            $this->weightType->unit = $this->validateString($data['unit'], 'unit', 20, true);
            $this->weightType->value = $this->validateInteger($data['value'], 'value');
            
            
            $name = $this->weightType->name;
            if(isset($existingData['weightTypeMap'][$name])){
                $this->weightType->weight_type_id = $existingData['weightTypeMap'][$name];
                $this->isExistingData = true;
            }
            else{
                $this->weightType->weight_type_id = $this->validateInteger($data['weight_type_id'] ?? Null, 'weight_type_id', 0);
                if(!count($this->errors)){
                    $this->weightType->name = $name;
                    if(!$this->isExistingData){
                        $this->weightType->name = $this->weightType->name;
                    }
                }
            };
        }else{
            $this->addError('name', 'Weight Type name is required');
        }
    }
    ...
    private function validateString($value, string $field, int $maxWeight, bool $isMandatory = false): ?string
    {
        $value = $this->fixTextEncoding($value, $field);
        if ($isMandatory && ($value === null || $value === '') && !isset($this->rawData['product_id'])) {
            $this->addError($field, 'is mandatory');
        ...
        $s = trim($value);
        if (strlen($s) > $maxWeight) {
            $s = substr($s, 0, $maxWeight);
        }
        if (in_array($field, $this->requiredFields) && (($value == '') || $value === null)) {
            $this->addError($field, 'is mandatory');
            return null;
    ...
    {
        return [
            'weightType' => (array)$this->weightType,
        ];
    }
    ...
    {
        if (!empty($this->weightType->name)) {
            return 'weight_type_' . $this->weightType->name;
        }
    ...
    public function getWeightTypeUniqueIdentifier(): string
    {
        // unique identifi group name and language code.
        if (!empty($this->weightType->name)) {
            return 'weightType_' . $this->weightType->name;
        }
