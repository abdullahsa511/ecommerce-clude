...
class PostTagDataValidation
{
    ...
    public function __construct(array $data, array $mediaPaths, array $existingData = ["postTagMap" => [], 'postTagIds' => []])
    {
        $this->rawData = $data;

        $this->post_tag = new stdClass();

        $image_path = $mediaPaths['image_path'] ?? '/media/post-tag/image/';

        if(isset($data['post_tag_id'])) {
            $this->post_tag->post_tag_id = $this->validateInteger($data['post_tag_id'], 'name', 0);
            if(isset($existingData['postTagIds'][$data['post_tag_id']])) {
                $this->isExistingData = true;
            }
        }
        // ATTRIBUTE TABLE
        if(isset($data['name']) && !empty($data['name']) && $data['name']){
            $this->post_tag->name = $this->validateString($data['name'], 'name', 191);
            $this->post_tag->slug = $this->validateSlug($data['slug']) ?? null;
            $this->post_tag->description = $this->validateString($data['description'], 'description', 1000);
            $this->post_tag->image = $this->validateJson($image_path.$data['image'], 'image') ?? null;
            $this->post_tag->post_id = $this->validateInteger($data['post_id'], 'post_id') ?? null;
            
            
            $name = $this->post_tag->name;
            if(isset($existingData['postTagMap'][$name])){
                $this->post_tag->post_tag_id = $existingData['postTagMap'][$name];
                $this->isExistingData = true;
            }
            else{
                $this->post_tag->post_tag_id = $this->validateInteger($data['post_tag_id'] ?? Null, 'post_tag_id', 0);
                if(!count($this->errors)){
                    $this->post_tag->name = $name;
                    if(!$this->isExistingData){
                        $this->post_tag->name = $this->post_tag->name;
                    }
                }
            };
        }else{
            $this->addError('name', 'Post tag name is required');
        }
    }
    ...
    {
        if (!empty($this->post_tag->name)) {
            return 'posttag_' . $this->post_tag->name;
        }
    ...
    public function getPostTagUniqueIdentifier(): string
    {
