...
export default {
  ...
  methods: {
    ...
    countDigitsInRange(formatted, start, end) {
      return this.normalizePhoneDigits(
        String(formatted || "").slice(start, end)
      ).length;
    },
    onPhoneKeydown(e) {
      const allowedKeys = [
      ...
      if (/^\d$/.test(e.key)) {
        ...
        const formatted = this.phoneNumber;
        const digits = this.normalizePhoneDigits(formatted);
        const start = input.selectionStart ?? formatted.length;
        const end = input.selectionEnd ?? start;
        const selectedDigitCount = this.countDigitsInRange(
          formatted,
          start,
          end
        );
        if (digits.length - selectedDigitCount >= 10) {
          e.preventDefault();
          this.phoneError = "Use at most 10 digits.";
          this.message = "";
          return;
        }
        if (this.phoneError === "Use at most 10 digits.") {
          this.phoneError = "";
        }
        return;
      }
    ...
    setPhone(raw) {
      const all = this.normalizePhoneDigits(raw);
      ...
      this.phoneError =
        all.length > 10 ? "Use at most 10 digits." : "";
      this.message = "";
    },
    async submitCallRequest() {
      ...
      if (phoneDigits.length < 9 || phoneDigits.length > 10) {
        this.phoneError = "Enter a valid 9–10 digit Australian number";
        return;
      }
      ...
      try {
        ...
        if (this.page === 'manage_pinboard') {
          window.open(`/pinboards/phone-call-request/${this.pinboardUuid}`, '_blank');    
          this.$emit("submit-success");
          this.closeModal();     
        }else{
          window.location.href = `/pinboards/phone-call-request/${this.pinboardUuid}`;
        }
        // this.$emit("submit-success");
        // this.closeModal();
      } catch (e) {
                        ...
                        <div class="modal-body">
                            <div class="crm-inner sendToSell-modal-body">
                                ...
                                <div class="crm-field-wrap">
                                    <div class="crm-phone-input" :class="{ 'has-error': phoneError }">
                                        ...
                                        <input
                                            ...
                                            @paste.prevent="setPhone($event.clipboardData.getData('text'))"
                                        />
                                    </div>
                                ...
                                <div v-if="message" class="crm-message" role="alert" aria-live="polite">
                                    <div class="alert alert-danger mb-0 text-start">{{ message }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
