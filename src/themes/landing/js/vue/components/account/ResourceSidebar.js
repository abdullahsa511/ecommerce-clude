export default {
  ...
  computed: {
    ...
    isContextVisibleFilter(){
      return ['images', 'finishes', 'textiles','models'].includes(this.resourceType);
    },
    isContextVisible() {
    ...
    isResetVisible() {
      ...
      if (!['images', 'finishes', 'textiles', 'models'].includes(this.resourceType)) {
        return false;
      }
    ...
    isCategoryVisible() {
      if (['models'].includes(this.resourceType)) {
        return true;
      }
     return this.isContextSelected && ['product', 'showrooms', 'brand', 'type'].includes(this.localFilter.context) && !['documents'].includes(this.resourceType);
    },
    isAutocompleteVisible() {
      return this.isContextSelected && (this.localFilter.context !== 'product' || this.isCategorySelected || ['documents','models'].includes(this.resourceType));
    }
  },
  methods: {
    ...
    handleFilter() {
      if (['models','images'].includes(this.resourceType)) {
   ...
   async handleResetFilters() {
      ...
      if (this.resourceType === 'models') {
        await this.$store.dispatch('loadResource', { resource_type: 'models', force: true, reload:true});
      }else{
  ...
  template: /* html */ `
    <div class="th-sidebar-container pr-md-40" id="th-resource-sidebar-sticky">
      ...
      <form class="th-from-margin th-sidebar-form" id="filter-resource-form">
        ...
        <button v-show="isContextVisibleFilter" type="button" id="filter-resource-button" @click.prevent="handleFilter"
