...
export default {
    ...
    computed: {
        storeProducts() {
            // console.log('storeProducts SearchList', this.$store.getters.products);
    ...
    methods: {
        ...
        async handleApplyFilters(filter) {
            // update the global search results query
    ...
    <div id="vue-app-product-desk-modesty-panel">
        ...
        <div class="row th-products-list-wrapper">
            <div class="th-item-product col-6 col-sm-6 col-md-4 col-lg-4" v-for="(product, index) in storeProducts" :key="product.product_id">
                <div class="th-img-container th-cateogry-product-image">
                    <a :href="product.href" class="justify-content-center">
                        <img :src="product.dataSrc" alt="Product Image" />
                    </a>
                </div>

                <div class="th-add-to-pinboard position-absolute top-right-30" @click="handleAddToPinboard(product, index)"
                 :data-id="product.reference" data-model="product" :data-title="product.title" :data-description="product.description" :data-image="product.image">
